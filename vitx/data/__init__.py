...
def get_dataset(
    data_config: DataConfig, split: str = "train", no_transform: bool = False
):

    dataset = DATASET_REGISTRY[data_config.dataset].from_config(
        data_config=data_config, split=split, no_transform=no_transform
    )
    return dataset


# automatically import any Python files in the data/ directory
import_all_modules(FILE_ROOT, "vitx.data")
