...
class MultiModalTransformer(nn.Module):
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(scale * torch.randn((seq_size) * 2 + 2, embed_dim))
        ...
        self.vision_projection = nn.Parameter(scale * torch.randn(embed_dim, projection_dim))
        self.text_projection = nn.Parameter(scale * torch.randn(embed_dim, projection_dim))
    ...
    def forward(
        ...
        x = torch.cat(
            [
                self.vision_class_embedding.to(vision_embed.dtype) + torch.zeros(vision_embed.shape[0], 1, vision_embed.shape[-1], dtype=vision_embed.dtype, device=vision_embed.device),
                vision_embed,
                self.text_class_embedding.to(text_embed.dtype) + torch.zeros(text_embed.shape[0], 1, text_embed.shape[-1], dtype=text_embed.dtype, device=text_embed.device),
                text_embed,
